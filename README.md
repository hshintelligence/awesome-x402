...
- [AgentScrape](https://agent-scrape.healingsunhaven.workers.dev) — Pay-per-call web scraping for AI agents — no signup, no API keys, just USDC. 6 paid tools on Base mainnet: `scrape_webpage` (markdown/html/text/json), `extract_structured_data` (AI-powered with natural-language prompts and JSON schemas), `screenshot_webpage`, `extract_metadata` (OG, Twitter, JSON-LD), `create_browser_session`, `run_workflow` (multi-step atomic execution). $0.001–$0.008 USDC per call. Two surfaces — HTTP API + MCP Streamable HTTP. Free tier: 10 calls/wallet/30 days. Cloudflare Workers. ([GitHub](https://github.com/hshintelligence/agent-scrape)) ([MCP](https://agent-scrape.healingsunhaven.workers.dev/mcp))
